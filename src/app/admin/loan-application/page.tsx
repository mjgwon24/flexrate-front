...
import {LoanApplicationTableRow, useLoanApplicationsQuery} from '@/hooks/useLoanApplicationsQuery';
...
const LOAN_APPLICATION_COLUMN_METAS = [
    ...
    {title: '신청일', dataIndex: 'applicationDate', key: 'applicationDate', width: 120, editable: false},
    {title: '신청자', dataIndex: 'applicantName', key: 'applicantName', width: 90, editable: false},
    {title: '대출 가능 한도', dataIndex: 'loanLimit', key: 'loanLimit', width: 120, editable: false},
    {
        title: '초기 대출 금리',
...
const AdminLoanApplicationPage = () => {
     ...
     */
        // 임시 필터 변경 핸들러
    const handleTempFilterChange = <K extends keyof LoanFilterType>(key: K, value: LoanFilterType[K]) => {
            setTempFilters((prev) => ({
                ...prev,
                [key]: value,
            }));
    ...
    // 대출 상태 변경 핸들러
    const handleStatusChange = (newValue: string, dataIndex?: string, record?: LoanApplicationTableRow) => {
        console.log('상태 변경 요청:', {newValue, dataIndex, record});
    ...
    const handleModalOk = () => {
        ...
        statusChangeForm.validateFields()
            .then(values => {
                const {reason} = values;
